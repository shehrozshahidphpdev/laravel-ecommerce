            ...
            .then(data => {
              ...
              if (data.products && data.products.length > 0) {
                data.products.forEach(product => {
                  const category = product.category ? product.category.name : 'N/A';
                  const tag = product.tag ? product.tag.name : 'N/A';
                  const brand = product.brand ? product.brand.name : 'N/A';
                  const shortDesc = product.short_description
                    ? product.short_description.substring(0, 30) + (product.short_description.length > 30 ? '...' : '')
                    : 'N/A';
                  const desc = product.description
                    ? product.description.replace(/<[^>]*>/g, '').substring(0, 30) + (product.description.length > 30 ? '...' : '')
                    : 'N/A';
                  const discPrice = product.discounted_price ?? '—';
                  const sku = product.sku ?? 'N/A';
                  const qty = product.quantity ?? 'N/A';
                  const views = product.views_count ? product.views_count.toLocaleString() : '0';

                  const onSaleBadge = product.on_sale
                    ? `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">Sale</span>`
                    : `<span class="text-gray-400 dark:text-gray-600">—</span>`;

                  const statusBadge = product.is_active == 1
                    ? `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Active</span>`
                    : `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400"><span class="w-1.5 h-1.5 rounded-full bg-red-500"></span> Inactive</span>`;

                  const featuredBadge = product.is_featured
                    ? `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400">Featured</span>`
                    : `<span class="text-gray-400 dark:text-gray-600">—</span>`;

                  rows += `
                        <tr class="transition-colors duration-150 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          <td class="px-4 py-3 text-gray-500 dark:text-gray-400 font-medium">${product.id}</td>
                          <td class="px-4 py-3 text-gray-700 dark:text-gray-300">${category}</td>
                          <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">${tag}</span>
                          </td>
                          <td class="px-4 py-3 text-gray-700 dark:text-gray-300">${brand}</td>
                          <td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">${product.name}</td>
                          <td class="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-[160px] truncate">${shortDesc}</td>
                          <td class="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-[160px] truncate">${desc}</td>
                          <td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">${product.original_price}</td>
                          <td class="px-4 py-3 font-medium text-emerald-600 dark:text-emerald-400">${discPrice}</td>
                          <td class="px-4 py-3 text-gray-500 dark:text-gray-400 font-mono text-xs">${sku}</td>
                          <td class="px-4 py-3 text-gray-700 dark:text-gray-300">${qty}</td>
                          <td class="px-4 py-3">${onSaleBadge}</td>
                          <td class="px-4 py-3">${statusBadge}</td>
                          <td class="px-4 py-3">${featuredBadge}</td>
                          <td class="px-4 py-3 text-gray-500 dark:text-gray-400">${views}</td>
                          <td class="px-4 py-3">
                            <div class="flex items-center gap-1.5">
                              ...
                              <a href="products/${product.id}" title="show"
                                class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-600 dark:hover:bg-emerald-500/10 dark:hover:border-emerald-500/40 dark:hover:text-emerald-400 transition-colors duration-150">
                                <svg width="15" height="15" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10 3C5 3 1.73 7.11 1 10c.73 2.89 4 7 9 7s8.27-4.11 9-7c-.73-2.89-4-7-9-7Zm0 11a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm0-2.5A1.5 1.5 0 1 0 10 8a1.5 1.5 0 0 0 0 3Z"/>
                                </svg>
                              </a>
                            </div>
                          </td>
                ...
                rows = `<tr><td colspan="16" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No products found</td></tr>`;
                paginationContainer.innerHTML = '';
              }
