    ...
    <div class="mx-auto max-w-5xl">
      ...
      <div class="mb-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Tags</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Edit a Product Tag</p>
          </div>
        </div>
      </div>
      <div
                  ...
                  <select id="color_id" name="color_id"
                    ...
                    @foreach ($colors as $color)
                      <option value="{{ $color->id }}" data-color="{{ $color->hex_code }}"
                        style="background-color: {{ $color->hex_code }}" {{ $tag->color_id === $color->id ? 'selected' : '' }}>
                        {{  $color->hex_code }}
                      </option>
                    @endforeach
                  </select>
  ...
  @push('script')
    <script>
      document.addEventListener('DOMContentLoaded', function () {
        ...
        const previousColor = {{ Js::from($setInitialColor) }};
        colorPreview.style.backgroundColor = previousColor;
