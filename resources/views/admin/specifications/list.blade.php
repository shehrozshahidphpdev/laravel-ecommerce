<x-admin.app-layout title="admin - specifications">
  <div class="px-4 py-6 sm:px-6 lg:px-8">
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <div class="border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-900">
        <div class="flex flex-row gap-4 justify-between items-center">
          ...
          <a href="{{ route('specifications.create') }}" class="
    ...
    <div class="overflow-x-auto">
      <table class="w-full table-fixed">
        ...
        <tbody class="table-body divide-y divide-gray-100 bg-white dark:divide-gray-100 dark:bg-gray-900">
          @foreach($labels as $label)
            <tr class="transition-colors  dark:hover:bg-gray-800/40">
              <td class="px-6 py-4">
                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $label->id }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $label->label }}</span>
              </td>

              {{-- delete btn --}}
              <td class="px-6 py-4">
                <div class="flex flex-row gap-2">
                  <x-form action="{{ route('specifications.destroy', $label->id) }}" method="DELETE">
                    <span
                  ...
                  {{-- edit btn --}}
                  <a href="{{ route('specifications.edit', $label->id) }}">
                    <span
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      input.addEventListener('keyup', function () {
        ...
        let rows = '';
        fetch("{{ route('specifications.search') }}", {
          method: 'POST',
          headers: {
            'X-CSRF-TOKEN': token,
          ...
          .then(function (data) {
            ...
            if (data.status && data.labels.length > 0) {
              data.labels.forEach(label => {
                rows += `    <tr class="transition-colors  dark:hover:bg-gray-800/40">
              <td class="px-6 py-4">
                <span class="text-sm text-gray-700 dark:text-gray-300">${label.id}</span>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div class="flex flex-row gap-2">
                  <form action="admin/specifications/${label.id}" method="POST">
                  ...
                  <a href="admin/specifications/${label.id}">
                    <span
                      class=" inline-flex items-center rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold                                                                                                                             
                                                                                                                                                                                                      text-sky-800 dark:bg-sky-500/20 dark:text-sky-400">
                      <svg
              ...
              rows = `<tr class="transition-colors  dark:hover:bg-gray-800/40">
                <td class="px-6 py-4">
                  <span class="text-lg text-gray-700 dark:text-gray-300">Sorry No Data Found</span>
                </td>
              </tr>`;
            }
