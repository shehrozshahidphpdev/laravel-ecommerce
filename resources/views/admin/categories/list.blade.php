      ...
      <div class="border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-900">
        <div class="flex flex-row gap-4 justify-between items-center">
          ...
          <div class="flex items-center gap-3">
            <input type="text" x-model="search" placeholder="Search..." class="
            h-11 w-full xl:w-[300px]
            rounded-lg border
            px-2 py-2.5 pl-5 text-sm
            shadow-sm
        ...
        <table class="w-full table-fixed">
          <thead>
            <tr class="border-b border-gray-200 bg-gray-50/50 dark:border-gray-700 dark:bg-gray-800/50">
              <th
                class="w-2 px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">
                #
              </th>
              <th
          ...
          <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-100 dark:bg-gray-900">
            @foreach($categories as $category)
              ...
              <tr class="transition-colors  dark:hover:bg-gray-800/40">
                ...
                <td class="px-6 py-4">
                  <span class="text-sm text-gray-700 dark:text-gray-300">{{ $category->parent->name ?? 'N/A' }}</span>
                </td>
                {{-- image --}}
                <td class="px-6 py-4">
                  <div class="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                    <img
                      src="{{ $category->image ? asset('storage/' . $category->image) : asset('assets/photos/No_Image_Available.jpg') }}"
                      alt="category image" class="h-full w-full object-cover" />
                  </div>
                </td>
                {{-- category icon --}}
                <td class="px-6 py-4">
                  <div class="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                    <img
                      src="{{ $category->category_icon ? asset('storage/' . $category->category_icon) : asset('assets/photos/No_Image_Available.jpg') }}"
                ...
                <td class="px-6 py-4">
                  <div class="flex flex-row gap-2">
                    <x-form action="{{ route('categories.destroy', $category->id) }}" method="DELETE">
                      <span
